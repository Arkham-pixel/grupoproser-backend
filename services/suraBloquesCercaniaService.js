/**
 * Geocodificación y bloques de cercanía para Seguros Sura (solo ARNALD).
...
export function hashDireccionSura(direccion = '', ciudad = '', departamento = '') {
...
export function construirQueryGeocodeSura(caso = {}) {
 ...
 */
export function esDireccionPredioGeocodableSura(direccion = '') {
...
export function casoTieneCoordsValidas(caso) {
  ...
  return (
    u &&
    esDireccionPredioGeocodableSura(caso?.direccionPredio) &&
...
export function casoNecesitaGeocode(caso, { force = false } = {}) {
  if (!esDireccionPredioGeocodableSura(caso?.direccionPredio)) return false;
  if (force) return true;
  const u = caso?.ubicacionPredio;
  if (!u) return true;
  if (u.geocodeStatus === 'stale' || u.geocodeStatus === 'pending') return true;
  if (u.geocodeStatus === 'failed') return true;
  const hash = hashDireccionSura(caso.direccionPredio, caso.ciudad, caso.departamento);
...
export function casoYaInspeccionadoSura(caso = {}) {
...
export async function geocodeCasosSuraPendientes({ limit = 40, force = false } = {}) {
  const lim = Math.min(Math.max(Number(limit) || 40, 1), 100);
  const casos = await SegurosSuraCaso.find({}).sort({ updatedAt: -1 }).lean();
  ...
  for (const caso of pendientes) {
    ...
    if (!esDireccionPredioGeocodableSura(caso.direccionPredio)) {
      await SegurosSuraCaso.findByIdAndUpdate(caso._id, {
        $set: {
 ...
 */
export async function aplicarUbicacionesPredioSura(items = []) {
  const aplicados = [];
  for (const item of items) {
    ...
    const doc = await SegurosSuraCaso.findByIdAndUpdate(id, update, { new: true }).lean();
    if (doc) {
      aplicados.push({
...
export async function obtenerBloquesCercaniaSura({
  radioKm = 2.5,
  ciudad = '',
  estado = '',
} = {}) {
  ...
  const casos = await SegurosSuraCaso.find(filtro).sort({ createdAt: -1 }).lean();
  const ubicados = [];
  const sinUbicar = [];
