...
export function fechaBaseInactividadSura(caso) {
  return (
    parseFechaSura(caso?.fechaUltimoDocumento) ||
    parseFechaSura(caso?.updatedAt) ||
    parseFechaSura(caso?.createdAt)
  );
}

export function evaluarAlertaInactividadSura(caso, ahora = new Date()) {
  if (!caso || esEstadoSuraCerrado(caso.estado)) return null;

  const base = fechaBaseInactividadSura(caso);
  if (!base) return null;

  const dias = diasCalendarioEntreFechas(base, ahora);
  if (dias == null || dias < DIAS_RECORDATORIO_INACTIVIDAD_SURA) return null;

  const retraso = dias - DIAS_RECORDATORIO_INACTIVIDAD_SURA;
  const origen = parseFechaSura(caso.fechaUltimoDocumento)
    ? 'último documento'
    : 'última actualización del caso';

  return {
    etapaId: 'recordatorioInactividadSura',
    nombre: 'Recordatorio inactividad / documentos',
    fase: 0,
    prioridad: dias >= DIAS_RECORDATORIO_INACTIVIDAD_SURA * 2 ? 'ALTA' : 'MEDIA',
    mensaje: `Han pasado ${dias} días desde el ${origen} sin movimiento. Revisar documentación y estado del caso.`,
    transcurrido: dias,
    limite: DIAS_RECORDATORIO_INACTIVIDAD_SURA,
    retraso: Math.max(0, retraso),
    horasLimite: DIAS_RECORDATORIO_INACTIVIDAD_SURA * 24,
    horasTranscurridas: dias * 24,
    etiquetaLimite: `${DIAS_RECORDATORIO_INACTIVIDAD_SURA} días calendario`,
    tipo: 'RECORDATORIO_INACTIVIDAD_SURA',
    accion: 'Revisar el caso, subir documentos pendientes y actualizar el estado',
  };
}

function mapearArchivosCasoSura(caso) {
...
export function generarAlertasCasoSura(caso, ahora = new Date()) {
  if (!caso || esEstadoSuraCerrado(caso.estado)) return [];
  const alerta = evaluarAlertaInactividadSura(caso, ahora);
  if (!alerta) return [];
  return [
    {
      ...alerta,
...
export async function obtenerTodasAlertasSura() {
  ...
  for (const caso of casos) {
    const alertas = generarAlertasCasoSura(caso, ahora);
    ...
    porCaso.push({
      ...
      totalAlertas: alertas.length,
      alertas,
      archivos: mapearArchivosCasoSura(caso),
      inactividad:
        dias != null
          ? {
              actividad: parseFechaSura(caso.fechaUltimoDocumento)
                ? 'Último documento'
                : 'Última actualización',
              dias,
              estado: dias >= DIAS_RECORDATORIO_INACTIVIDAD_SURA * 2 ? 'CRÍTICO' : 'ALTO',
            }
          : null,
    });
  }
...
export async function obtenerAlertasSuraPorAjustadores() {
  const todas = await obtenerTodasAlertasSura();
  ...
  for (const caso of todas.casos || []) {
    ...
    bucket.casos.push(casoSuraAFormatoEmail(caso));
...
export async function obtenerAlertasAjustadorSura(codigoAjustador) {
  const agrupadas = await obtenerAlertasSuraPorAjustadores();
...
export function debeEnviarRecordatorioEmailSura(responsable, ahora = new Date()) {
  const ultima = responsable?.fchaUltimoRecordatorioAlertasSura;
...
export async function enviarAlertasSuraAjustador(codigoAjustador, opciones = {}) {
  const forzar = opciones.forzar === true;
  const codigo = String(codigoAjustador || '').trim();
  console.log('📧 Enviando alertas Seguros Sura a:', codigo, forzar ? '(forzado)' : '');

  const alertas = await obtenerAlertasAjustadorSura(codigo);
  if (!alertas.casosConAlertas) {
    return { success: true, message: 'No hay alertas Sura para enviar', omitido: false };
  }
  ...
  if (!forzar && !debeEnviarRecordatorioEmailSura(responsable)) {
    const diasDesde = diasCalendarioEntreFechas(responsable.fchaUltimoRecordatorioAlertasSura);
    return {
      success: true,
      omitido: true,
      message: `Recordatorio Sura omitido: se reenvía cada ${DIAS_ENTRE_RECORDATORIOS_EMAIL} días`,
  ...
  const datosEmail = {
    modulo: 'sura',
    numeroCaso: `ALERTAS-SURA-${codigo}`,
    nombreResponsable: responsable.nmbrRespnsble || codigo,
    emailResponsable: responsable.email,
    aseguradora: 'Seguros Sura',
    asegurado: 'Ajustador Seguros Sura',
    fechaAsignacion: new Date().toLocaleDateString('es-CO'),
    quienAsigna: 'Sistema Seguros Sura',
    emailQuienAsigna: 'sistema@proserpuertos.com.co',
    observaciones: `Tienes ${alertas.casosConAlertas} casos Seguros Sura con alertas pendientes`,
    alertas,
    archivosConRuta: alertas.archivosConRuta || [],
  };

  const resultado = await enviarEmailAlertasSura(
  ...
  if (resultado?.success !== false) {
    await Responsable.updateOne(
      { _id: responsable._id },
      { $set: { fchaUltimoRecordatorioAlertasSura: new Date() } }
    );
  }
...
export async function enviarAlertasTodosSura(opciones = {}) {
  console.log('📧 Enviando alertas Seguros Sura a todos los ajustadores...');
  const agrupadas = await obtenerAlertasSuraPorAjustadores();
  const resultados = [];

  for (const item of agrupadas.ajustadores) {
    try {
      const resultado = await enviarAlertasSuraAjustador(item.codigoResponsable, opciones);
      resultados.push({
      ...
      console.error(`❌ Error enviando alertas Sura a ${item.codigoResponsable}:`, error);
      resultados.push({
        ajustador: item.codigoResponsable,
        success: false,
        omitido: false,
