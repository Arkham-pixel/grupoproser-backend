/**
 * Envío masivo: correo de apertura GRUPO PROSER → asegurados Seguros Allianz.
 *
 * Uso:
 *   node scripts/enviarCorreoMasivoAperturaAllianz.js              # dry-run
 *   node scripts/enviarCorreoMasivoAperturaAllianz.js --test=tu@correo.com
 *   node scripts/enviarCorreoMasivoAperturaAllianz.js --apply
...
const LOGO_ALLIANZ = path.join(ASSETS_EMAIL, 'logo-allianz.png');
const LOCK_PATH = path.join(__dirname, '.lock_apertura_allianz');

const CONTACTO_CORREO = 'jmhincapie@proserpuertos.com.co';
const CONTACTO_TELEFONO = '3146511524';
const BCC = 'jmhincapie@proserpuertos.com.co';

const EXCLUIR_SINIESTROS = new Set(['231692920', '232020876', '232025418']);

const ASUNTO = 'GRUPO PROSER — Atención de su reclamación Seguros Allianz';

const CUERPO_TEXTO = `Estimado(a) asegurado(a):

Reciba un cordial saludo.

Le informamos que GRUPO PROSER ha sido designado por Seguros Allianz para acompañar la atención y gestión técnica de su reclamación, relacionada con el evento reportado.
...
GRUPO PROSER
Ajustadores designados por Seguros Allianz.
`;
...
function adjuntosLogos() {
  ...
  if (fs.existsSync(LOGO_ALLIANZ)) {
    attachments.push({
      filename: 'logo-allianz.png',
      path: LOGO_ALLIANZ,
      cid: 'logoAllianz',
      contentDisposition: 'inline',
    });
...
function cuerpoHtml() {
  ...
  const tieneAllianz = fs.existsSync(LOGO_ALLIANZ);
  ...
  const celdaAllianz = tieneAllianz
    ? `<img src="cid:logoAllianz" alt="Seguros Allianz" width="140" height="48" style="display:block;max-width:140px;height:auto;border:0;margin-left:auto;" />`
    : `<span style="color:#003781;font-size:15px;font-weight:bold;">Seguros Allianz</span>`;
            ...
            <td style="background:#ffffff;padding:18px 24px;border-bottom:1px solid #e5e7eb;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td align="right" valign="middle" width="50%">${celdaAllianz}</td>
                </tr>
              </table>
            </td>
          </tr>
          <tr>
            <td style="background:#003781;color:#ffffff;padding:12px 28px;font-size:13px;font-weight:bold;letter-spacing:0.02em;">
              Atención de reclamación · Seguros Allianz
            </td>
          </tr>
          <tr>
            <td style="padding:28px;font-size:14px;line-height:1.55;color:#1f2937;">
              ...
              <p style="margin:0 0 14px;">
                ...
                <strong>Seguros Allianz</strong> para acompañar la atención y gestión técnica de su
...
function adquirirLock() {
  if (fs.existsSync(LOCK_PATH)) {
    ...
    throw new Error(`Ya hay un envío Allianz en curso (lock: ${prev}). Espere o borre ${LOCK_PATH}`);
  }
...
async function main() {
  ...
  try {
    ...
    const casos = await AllianzListadoCaso.find({})
      .select(
        '_id correoAsegurado contactoAsegurado correoIntermediario contactoIntermediario asegurado siniestro fechaEmailAperturaAllianz'
      )
      .lean();
    ...
    for (const c of casos) {
      const siniestro = String(c.siniestro || '').replace(/\.0$/, '').trim();
      if (EXCLUIR_SINIESTROS.has(siniestro)) {
        excluidos += 1;
        continue;
      }
      ...
      for (const email of emails) {
        ...
        if (c.fechaEmailAperturaAllianz) row.yaEnviado = true;
        if (!row.asegurado) row.asegurado = c.asegurado || '';
        if (!row.siniestro) row.siniestro = siniestro;
      }
    }
    ...
    if (args.test) {
      ...
      const info = await deliverMail(
        ...
        { enqueue: false, tipo: 'allianz-apertura-test' }
      );
    ...
    if (!args.apply) {
      ...
      console.log('  node scripts/enviarCorreoMasivoAperturaAllianz.js --apply');
      const previewPath = path.join(__dirname, `_preview_apertura_allianz_${Date.now()}.json`);
      fs.writeFileSync(
        previewPath,
        JSON.stringify(
          destinatarios.map((d) => ({
    ...
    const logPath = path.join(__dirname, `_log_apertura_allianz_${Date.now()}.json`);
