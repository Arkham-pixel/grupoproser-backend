...
/**
 * Casos BBVA CAT (módulo catastrófico estilo Alfa).
 * Puede alimentarse desde Excel o sincronizarse desde Express (SiniestroExpress).
 */
const BbvaCatCasoSchema = new mongoose.Schema(
  {
    /** Formato BBVA-CAT-YYYY-MM-N (asignado al crear; no editable) */
    consecutivo: String,
    ...
    celular: String,
    /** Canal de radicación (BbvaCat, presencial, etc.) */
    ...
    /** Campos operativos CAT BBVA (hoja CAT_BBVA / exposición) */
    ...
    /**
     * Severidad CAT BBVA (Manual CAT Ajustadores): 1–6 para reporte de exposición.
    ...
    archivos: { type: [ArchivoBbvaCatSchema], default: [] },
  },
  {
    collection: 'gsk3cAppbbvaCatCasos',
    timestamps: true,
  }
);
...
const BbvaCatCaso = mongoose.model(
  'BbvaCatCaso',
  BbvaCatCasoSchema,
  'gsk3cAppbbvaCatCasos'
);

export default BbvaCatCaso;
