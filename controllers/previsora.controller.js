...
/** Claves de deduplicación CAT/Previsora.
...
const completarIdentificacionPrevisora = (payload = {}) => {
...
const obtenerMaxSecuencialPrevisora = async () => {
  const patron = /^PREVISORA-(\d{4})-(\d{2})-(\d+)$/i;
  const registros = await PrevisoraCaso.find({
...
/** Formato: PREVISORA-YYYY-MM-N (asignado solo al crear) */
const generarConsecutivoPrevisora = async () => {
  const ahora = new Date();
  const año = ahora.getFullYear();
  const mes = String(ahora.getMonth() + 1).padStart(2, '0');
  const maxSecuencial = await obtenerMaxSecuencialPrevisora();
...
const buildPrevisoraPayload = (data = {}, base = {}) => {
  const payload = {
...
/** Mapea un SiniestroExpress → campos Previsora (estructura Alfa). */
export const mapExpressAPrevisora = (express = {}) => ({
  ...
  estado: homologarEstadoPrevisora(express.estadoProceso),
...
const mergeImportacionPrevisora = (incomingPayload = {}, existente = {}) => {
  const campos = [
    'siniestro',
    'zc',
    'identificacion',
  ...
  out.estado = homologarEstadoPrevisora(out.estado);
...
export const crearCasoPrevisora = async (req, res) => {
  try {
    const payload = completarIdentificacionPrevisora(buildPrevisoraPayload(req.body));
    payload.consecutivo = await generarConsecutivoPrevisora();
    ...
    console.error('❌ Error al crear caso Previsora:', error);
    res.status(500).json({
      success: false,
      error: 'Error al guardar el caso Previsora',
...
export const listarCasosPrevisora = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Error al obtener los casos Previsora',
      detalle: error.message,
    });
  }
};
...
export const obtenerCasoPrevisora = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Error al obtener el caso Previsora',
      detalle: error.message,
    });
  }
};

export const actualizarCasoPrevisora = async (req, res) => {
  try {
    ...
    const payload = buildPrevisoraPayload(bodyFiltrado, base);
    if (!payload.consecutivo) {
      payload.consecutivo = base.consecutivo || (await generarConsecutivoPrevisora());
    }
    ...
    res.status(500).json({
      success: false,
      error: 'Error al actualizar el caso Previsora',
      detalle: error.message,
    });
  }
};

export const eliminarCasoPrevisora = async (req, res) => {
  try {
    ...
    await PrevisoraCaso.deleteOne({ _id: registro._id });
    res.json({ success: true, message: 'Caso Previsora eliminado correctamente' });
  } catch (error) {
    console.error('❌ Error al eliminar caso Previsora:', error);
    res.status(500).json({
      success: false,
      error: 'Error al eliminar el caso Previsora',
      detalle: error.message,
    });
  }
};
 ...
 */
export const importarCasosPrevisora = async (req, res) => {
  try {
    ...
    if (reemplazarTodo) {
      await PrevisoraCaso.deleteMany({});
    }

    const existentes = await PrevisoraCaso.find().lean();
    ...
    const año = ahora.getFullYear();
    const mes = String(ahora.getMonth() + 1).padStart(2, '0');
    let secuencial = await obtenerMaxSecuencialPrevisora();
    ...
    for (let i = 0; i < filas.length; i += 1) {
      ...
      try {
        const payloadBase = completarIdentificacionPrevisora(
          buildPrevisoraPayload({
            ...fila,
            estado: homologarEstadoPrevisora(fila.estado),
          })
        );
        ...
        if (existente) {
          const merge = mergeImportacionPrevisora(payloadBase, existente);
          if (!merge.consecutivo) {
            secuencial += 1;
            merge.consecutivo = `PREVISORA-${año}-${mes}-${secuencial}`;
          }

          const actualizado = await PrevisoraCaso.findByIdAndUpdate(existente._id, merge, {
            new: true,
          }).lean();
          ...
          secuencial += 1;
          payloadBase.consecutivo = `PREVISORA-${año}-${mes}-${secuencial}`;
          const creado = await PrevisoraCaso.create(payloadBase);
    ...
    console.error('❌ Error al importar casos Previsora:', error);
    res.status(500).json({
      success: false,
      error: 'Error al importar los casos Previsora',
      detalle: error.message,
    });
  }
};
...
const buildArchivoFromUpload = (req, etiqueta, { descripcion = '', orden = 0 } = {}) => {
  ...
  return {
    ...
    ruta: `/uploads/previsora/${file.filename}`,
    tamaño: file.size,
...
export const subirArchivoPrevisora = async (req, res) => {
  try {
    ...
    console.error('❌ Error subiendo archivo Previsora:', error);
    res.status(500).json({
      success: false,
...
/** PATCH /api/previsora/:id/archivos/:archivoId — descripción / etiqueta / orden */
export const actualizarArchivoPrevisora = async (req, res) => {
  try {
    const caso = await buscarCasoPorId(req.params.id);
    if (!caso) {
      return res.status(404).json({ success: false, error: 'Caso Previsora no encontrado' });
    ...
    console.error('❌ Error actualizando archivo Previsora:', error);
    res.status(500).json({
      success: false,
...
/** PUT /api/previsora/:id/archivos/orden — reordenar por lista de IDs */
export const reordenarArchivosPrevisora = async (req, res) => {
  try {
    const caso = await buscarCasoPorId(req.params.id);
    if (!caso) {
      return res.status(404).json({ success: false, error: 'Caso Previsora no encontrado' });
    ...
    console.error('❌ Error reordenando archivos Previsora:', error);
    res.status(500).json({
      success: false,
...
/** DELETE /api/previsora/:id/archivos/:archivoId */
export const eliminarArchivoPrevisora = async (req, res) => {
  try {
    const caso = await buscarCasoPorId(req.params.id);
    if (!caso) {
      return res.status(404).json({ success: false, error: 'Caso Previsora no encontrado' });
    ...
    if (archivo.ruta) {
      await deleteStoredFile(archivo.ruta).catch((err) => {
        console.warn('No se pudo eliminar archivo Previsora del almacenamiento:', err.message);
      });
    }
    archivo.deleteOne();
    await caso.save();
    ...
    console.error('❌ Error eliminando archivo Previsora:', error);
    res.status(500).json({
      success: false,
...
/** GET /api/previsora/alertas */
export const getAlertasPrevisora = async (_req, res) => {
  try {
    const data = await obtenerAlertasPrevisoraPorAjustadores();
    return res.json(data);
  } catch (error) {
    console.error('Error alertas Previsora:', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/** POST /api/previsora/alertas/enviar */
export const postEnviarAlertasPrevisoraTodas = async (req, res) => {
  try {
    const forzar = req.query.forzar === 'true' || req.body?.forzar === true;
    const data = await enviarAlertasTodosPrevisora({ forzar });
    return res.json(data);
  } catch (error) {
    console.error('Error enviando alertas Previsora (todas):', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/** POST /api/previsora/alertas/enviar/:ajustador */
export const postEnviarAlertasPrevisoraAjustador = async (req, res) => {
  try {
    ...
    const forzar = req.query.forzar === 'true' || req.body?.forzar === true;
    const data = await enviarAlertasPrevisoraAjustador(ajustador, { forzar });
    return res.json(data);
  } catch (error) {
    console.error('Error enviando alertas Previsora (ajustador):', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/**
 * Sincroniza casos desde SiniestroExpress → colección Previsora.
...
export const syncDesdeExpress = async (req, res) => {
  try {
    ...
    const filtroAseguradora = { aseguradora: { $regex: /previsora/i } };
    ...
    const existentes = await PrevisoraCaso.find({
      $or: [
    ...
    for (const exp of expressCasos) {
      try {
        const mapped = mapExpressAPrevisora(exp);
        const payloadBase = buildPrevisoraPayload({
          ...mapped,
          estado: homologarEstadoPrevisora(mapped.estado),
        });
        ...
        if (existente) {
          const merged = mergeImportacionPrevisora(payloadBase, existente);
          ...
          const actualizado = await PrevisoraCaso.findByIdAndUpdate(existente._id, merged, {
            new: true,
          }).lean();
          resumen.actualizados += 1;
          if (actualizado) {
    ...
    console.error('❌ Error al sincronizar Express → Previsora:', error);
