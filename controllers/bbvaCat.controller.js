...
/** Claves de deduplicación CAT/BbvaCat.
...
const completarIdentificacionBbvaCat = (payload = {}) => {
...
const obtenerMaxSecuencialBbvaCat = async () => {
...
/** Formato: BBVA-CAT-YYYY-MM-N (asignado solo al crear) */
const generarConsecutivoBbvaCat = async () => {
  const ahora = new Date();
  const año = ahora.getFullYear();
  const mes = String(ahora.getMonth() + 1).padStart(2, '0');
  const maxSecuencial = await obtenerMaxSecuencialBbvaCat();
  return `BBVA-CAT-${año}-${mes}-${maxSecuencial + 1}`;
};
...
const buscarCasoPorId = async (idParam) => {
  ...
  if (mongoose.Types.ObjectId.isValid(id)) {
    const porObjectId = await BbvaCatCaso.findById(id);
    if (porObjectId) return porObjectId;
  }
  return null;
};

const buildBbvaCatPayload = (data = {}, base = {}) => {
  const payload = {
  ...
  correoAsegurado: toStringOrNull(data.correoAsegurado, base.correoAsegurado ?? null),
  telefonoAsegurado: toStringOrNull(data.telefonoAsegurado, base.telefonoAsegurado ?? null),
  ...
  estado: homologarEstadoBbvaCat(toStringOrNull(data.estado, base.estado ?? 'CASO NUEVO')),
...
/** Mapea un SiniestroExpress → campos BBVA CAT (estructura Alfa). */
export const mapExpressABbvaCat = (express = {}) => ({
  ...
  estado: homologarEstadoBbvaCat(express.estadoProceso),
...
const mergeImportacionBbvaCat = (incomingPayload = {}, existente = {}) => {
  const campos = [
    'siniestro',
    'zc',
    'identificacion',
  ...
  out.estado = homologarEstadoBbvaCat(out.estado);
...
export const crearCasoBbvaCat = async (req, res) => {
  try {
    const payload = completarIdentificacionBbvaCat(buildBbvaCatPayload(req.body));
    payload.consecutivo = await generarConsecutivoBbvaCat();
    ...
    console.error('❌ Error al crear caso BBVA CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al guardar el caso BBVA CAT',
      detalle: error.message,
    });
  }
};

export const listarCasosBbvaCat = async (req, res) => {
  try {
    ...
    console.error('❌ Error al listar casos BBVA CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al obtener los casos BBVA CAT',
      detalle: error.message,
    });
  }
};

export const obtenerCasoBbvaCat = async (req, res) => {
  try {
    const documento = await buscarCasoPorId(req.params.id);
    if (!documento) {
      return res.status(404).json({ success: false, error: 'Caso BBVA CAT no encontrado' });
    }
    res.json({ success: true, data: documento });
  } catch (error) {
    console.error('❌ Error al obtener caso BBVA CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al obtener el caso BBVA CAT',
      detalle: error.message,
    });
  }
};

export const actualizarCasoBbvaCat = async (req, res) => {
  try {
    ...
    const payload = buildBbvaCatPayload(bodyFiltrado, base);
    if (!payload.consecutivo) {
      payload.consecutivo = base.consecutivo || (await generarConsecutivoBbvaCat());
    }
    ...
    console.error('❌ Error al actualizar caso BBVA CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al actualizar el caso BBVA CAT',
      detalle: error.message,
    });
  }
};

export const eliminarCasoBbvaCat = async (req, res) => {
  try {
    ...
    await BbvaCatCaso.deleteOne({ _id: registro._id });
    res.json({ success: true, message: 'Caso BBVA CAT eliminado correctamente' });
  } catch (error) {
    console.error('❌ Error al eliminar caso BBVA CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al eliminar el caso BBVA CAT',
      detalle: error.message,
    });
  }
};
 ...
 */
export const importarCasosBbvaCat = async (req, res) => {
  try {
    ...
    if (reemplazarTodo) {
      await BbvaCatCaso.deleteMany({});
    }

    const existentes = await BbvaCatCaso.find().lean();
    ...
    const año = ahora.getFullYear();
    const mes = String(ahora.getMonth() + 1).padStart(2, '0');
    let secuencial = await obtenerMaxSecuencialBbvaCat();
    ...
    for (let i = 0; i < filas.length; i += 1) {
      ...
      try {
        const payloadBase = completarIdentificacionBbvaCat(
          buildBbvaCatPayload({
            ...fila,
            estado: homologarEstadoBbvaCat(fila.estado),
          })
        );
        ...
        if (existente) {
          const merge = mergeImportacionBbvaCat(payloadBase, existente);
          if (!merge.consecutivo) {
            secuencial += 1;
            merge.consecutivo = `BBVA-CAT-${año}-${mes}-${secuencial}`;
          }

          const actualizado = await BbvaCatCaso.findByIdAndUpdate(existente._id, merge, {
            new: true,
          }).lean();
          ...
          secuencial += 1;
          payloadBase.consecutivo = `BBVA-CAT-${año}-${mes}-${secuencial}`;
          const creado = await BbvaCatCaso.create(payloadBase);
    ...
    console.error('❌ Error al importar casos BBVA CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al importar los casos BBVA CAT',
      detalle: error.message,
    });
  }
};
...
const buildArchivoFromUpload = (req, etiqueta, { descripcion = '', orden = 0 } = {}) => {
  ...
  return {
    ...
    ruta: `/uploads/bbva-cat/${file.filename}`,
    tamaño: file.size,
...
/** POST /api/bbva-cat/:id/archivos */
export const subirArchivoBbvaCat = async (req, res) => {
  try {
    if (!req.file) {
...
/** PATCH /api/bbva-cat/:id/archivos/:archivoId — descripción / etiqueta / orden */
export const actualizarArchivoBbvaCat = async (req, res) => {
  try {
    const caso = await buscarCasoPorId(req.params.id);
    if (!caso) {
      return res.status(404).json({ success: false, error: 'Caso BBVA CAT no encontrado' });
    ...
    console.error('❌ Error actualizando archivo BbvaCat:', error);
    res.status(500).json({
      success: false,
...
/** PUT /api/bbva-cat/:id/archivos/orden — reordenar por lista de IDs */
export const reordenarArchivosBbvaCat = async (req, res) => {
  try {
    const caso = await buscarCasoPorId(req.params.id);
    if (!caso) {
      return res.status(404).json({ success: false, error: 'Caso BBVA CAT no encontrado' });
    ...
    console.error('❌ Error reordenando archivos BbvaCat:', error);
    res.status(500).json({
      success: false,
...
/** DELETE /api/bbva-cat/:id/archivos/:archivoId */
export const eliminarArchivoBbvaCat = async (req, res) => {
  try {
    const caso = await buscarCasoPorId(req.params.id);
    if (!caso) {
      return res.status(404).json({ success: false, error: 'Caso BBVA CAT no encontrado' });
    ...
    console.error('❌ Error eliminando archivo BbvaCat:', error);
    res.status(500).json({
      success: false,
...
/** GET /api/bbva-cat/alertas */
export const getAlertasBbvaCat = async (_req, res) => {
  try {
    const data = await obtenerAlertasBbvaCatPorAjustadores();
    return res.json(data);
  } catch (error) {
    console.error('Error alertas BBVA CAT:', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/** POST /api/bbva-cat/alertas/enviar */
export const postEnviarAlertasBbvaCatTodas = async (req, res) => {
  try {
    const forzar = req.query.forzar === 'true' || req.body?.forzar === true;
    const data = await enviarAlertasTodosBbvaCat({ forzar });
    return res.json(data);
  } catch (error) {
    console.error('Error enviando alertas BBVA CAT (todas):', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/** POST /api/bbva-cat/alertas/enviar/:ajustador */
export const postEnviarAlertasBbvaCatAjustador = async (req, res) => {
  try {
    ...
    const forzar = req.query.forzar === 'true' || req.body?.forzar === true;
    const data = await enviarAlertasBbvaCatAjustador(ajustador, { forzar });
    return res.json(data);
  } catch (error) {
    console.error('Error enviando alertas BBVA CAT (ajustador):', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/**
 * Sincroniza casos desde SiniestroExpress → colección BBVA CAT.
...
export const syncDesdeExpress = async (req, res) => {
  try {
    ...
    for (const exp of expressCasos) {
      try {
        const mapped = mapExpressABbvaCat(exp);
        const payloadBase = buildBbvaCatPayload({
          ...mapped,
          estado: homologarEstadoBbvaCat(mapped.estado),
        });
        ...
        if (existente) {
          const merged = mergeImportacionBbvaCat(payloadBase, existente);
          ...
          const actualizado = await BbvaCatCaso.findByIdAndUpdate(existente._id, merged, {
            new: true,
          }).lean();
          resumen.actualizados += 1;
          if (actualizado) {
    ...
    console.error('❌ Error al sincronizar Express → BbvaCat:', error);
...
/** POST /api/bbva-cat/geocode-pendientes */
export const postGeocodePendientesBbvaCat = async (req, res) => {
  try {
    const limit = req.body?.limit ?? req.query?.limit ?? 40;
    const force = req.body?.force === true || req.query?.force === 'true';
    const data = await geocodeCasosBbvaCatPendientes({ limit, force });
    return res.json({ success: true, data });
  } catch (error) {
    console.error('Error geocode pendientes BBVA CAT:', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/** POST /api/bbva-cat/ubicaciones-predio */
export const postUbicacionesPredioBbvaCat = async (req, res) => {
  try {
    const items = Array.isArray(req.body?.items) ? req.body.items : [];
    if (!items.length) {
      return res.status(400).json({ success: false, error: 'items[] requerido' });
    }
    const data = await aplicarUbicacionesPredioBbvaCat(items);
    return res.json({ success: true, data });
  } catch (error) {
    console.error('Error aplicando ubicaciones BBVA CAT:', error);
    return res.status(500).json({ success: false, error: error.message });
  }
};

/** GET /api/bbva-cat/bloques-cercania */
export const getBloquesCercaniaBbvaCat = async (req, res) => {
  try {
