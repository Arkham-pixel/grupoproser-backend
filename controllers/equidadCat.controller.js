...
const esPlaceholder = (valor) => {
  ...
  const t = String(valor)
    ...
    .toUpperCase()
    .replace(/\s+/g, ' ');
  return !t || /^(N\/?A|N\/?C|NA|NC|NULL|-|0|POR CONFIRMAR|SIN DATO|PENDIENTE)$/.test(t);
};
...
const completarIdentificacion = (payload = {}) => {
  ...
  else if (payload.numeroCasoCliente) payload.identificacion = String(payload.numeroCasoCliente);
  return payload;
};

const sincronizarCamposListado = (payload = {}) => {
  if (!payload.celular && payload.telefonoAsegurado) payload.celular = payload.telefonoAsegurado;
  if (!payload.telefonoAsegurado && payload.celular) payload.telefonoAsegurado = payload.celular;
  if (payload.valorAsegurado == null && payload.valorAseguradoInmueble != null) {
    payload.valorAsegurado = payload.valorAseguradoInmueble;
  }
  if (payload.valorAseguradoInmueble == null && payload.valorAsegurado != null) {
    payload.valorAseguradoInmueble = payload.valorAsegurado;
  }
  if (payload.valorLiquidado == null && payload.valorIndemnizado != null) {
    payload.valorLiquidado = payload.valorIndemnizado;
  }
  if (!payload.asignadoAAjustador && payload.ajustador) payload.asignadoAAjustador = 'SI';
  if (!payload.asignacion && payload.ajustador) payload.asignacion = 'SI';
  return payload;
};
...
const buildPayload = (data = {}, base = {}, { pisar = false } = {}) => {
  ...
  const payload = completarIdentificacion({
    ...
    numeroCasoCliente: pick(data.numeroCasoCliente, base.numeroCasoCliente ?? null),
    ...
    producto: pick(data.producto, base.producto ?? null),
    ...
    analista: pick(data.analista, base.analista ?? null),
    ...
    celular: pick(data.celular, base.celular ?? null),
    ...
    comentariosAnalista: pick(data.comentariosAnalista, base.comentariosAnalista ?? null),
    ciudad: pick(data.ciudad, base.ciudad ?? null),
    departamento: pick(data.departamento, base.departamento ?? null),
    asignacion: pick(data.asignacion, base.asignacion ?? null),
    asignadoAAjustador: pick(data.asignadoAAjustador, base.asignadoAAjustador ?? null),
    visita: pick(data.visita, base.visita ?? null),
    valorAsegurado: parseNumero(data.valorAsegurado, base.valorAsegurado ?? null),
    ...
    reservaDirecta: parseNumero(data.reservaDirecta, base.reservaDirecta ?? null),
    reservaGastos: parseNumero(data.reservaGastos, base.reservaGastos ?? null),
    diferenciaReserva: parseNumero(data.diferenciaReserva, base.diferenciaReserva ?? null),
    ...
    valorIndemnizado: parseNumero(data.valorIndemnizado, base.valorIndemnizado ?? null),
    deducibleMaxPct: parseNumero(data.deducibleMaxPct, base.deducibleMaxPct ?? null),
    tipoDeducible: pick(data.tipoDeducible, base.tipoDeducible ?? null),
    deducibleSmmlv: parseNumero(data.deducibleSmmlv, base.deducibleSmmlv ?? null),
    ...
    fechaAviso: pickFecha(data.fechaAviso, base.fechaAviso ?? null),
    ...
    fechaDefinicion: pickFecha(data.fechaDefinicion, base.fechaDefinicion ?? null),
    fechaUltimoDocumento: pickFecha(data.fechaUltimoDocumento, base.fechaUltimoDocumento ?? null),
    fechaCausacion: pickFecha(data.fechaCausacion, base.fechaCausacion ?? null),
    fechaGiro: pickFecha(data.fechaGiro, base.fechaGiro ?? null),
    estado: homologarEstadoEquidadCat(pick(data.estado, base.estado ?? 'CASO NUEVO') || 'CASO NUEVO'),
  ...
  return aplicarFechaAccionEstadoEquidadCat(
    sincronizarCamposListado(armarContactoAsegurado(armarContactoIntermediario(payload))),
    base
  );
};
...
const obtenerMaxSecuencial = async () => {
  ...
  const registros = await EquidadCatCaso.find({
...
const generarConsecutivo = async () => {
  ...
  return `EQUIDAD-CAT-${año}-${mes}-${max + 1}`;
};

export const crearCasoListadoEquidadCat = async (req, res) => {
  try {
    ...
    console.error('❌ Error al crear caso Equidad CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al guardar el caso del Equidad CAT',
      detalle: error.message,
    });
  }
};

export const listarCasosListadoEquidadCat = async (req, res) => {
  try {
    ...
    console.error('❌ Error al listar Equidad CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al obtener los casos del Equidad CAT',
      detalle: error.message,
    });
  }
};

export const obtenerCasoListadoEquidadCat = async (req, res) => {
  try {
    const documento = await EquidadCatCaso.findById(req.params.id);
    if (!documento) {
      return res.status(404).json({ success: false, error: 'Caso del listado no encontrado' });
    }
    res.json({ success: true, data: documento });
  } catch (error) {
    console.error('❌ Error al obtener Equidad CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al obtener el caso del Equidad CAT',
      detalle: error.message,
    });
  }
};

export const actualizarCasoListadoEquidadCat = async (req, res) => {
  try {
    const actual = await EquidadCatCaso.findById(req.params.id);
    if (!actual) {
      return res.status(404).json({ success: false, error: 'Caso del listado no encontrado' });
    }
    ...
    console.error('❌ Error al actualizar Equidad CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al actualizar el caso del Equidad CAT',
      detalle: error.message,
    });
  }
};

export const eliminarCasoListadoEquidadCat = async (req, res) => {
  try {
    const registro = await EquidadCatCaso.findById(req.params.id);
    if (!registro) {
      return res.status(404).json({ success: false, error: 'Caso del listado no encontrado' });
    }
    await EquidadCatCaso.deleteOne({ _id: registro._id });
    res.json({ success: true, message: 'Caso del listado eliminado correctamente' });
  } catch (error) {
    console.error('❌ Error al eliminar Equidad CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al eliminar el caso del Equidad CAT',
      detalle: error.message,
    });
  }
};
 ...
 */
export const importarCasosListadoEquidadCat = async (req, res) => {
  try {
    ...
    const existentes = await EquidadCatCaso.find().lean();
    const [inspectoresRaw, ajustadoresRaw] = await Promise.all([
      InspectorCatastrofico.find({}).lean(),
      AjustadorCatastrofico.find({}).lean(),
    ]);
    const inspectores = inspectoresRaw.filter((d) => catalogoPerteneceAModulo(d, 'equidadCat'));
    const ajustadores = ajustadoresRaw.filter((d) => catalogoPerteneceAModulo(d, 'equidadCat'));
    ...
    for (let i = 0; i < filas.length; i += 1) {
      ...
      try {
        ...
        const payload = buildPayload({
          ...
          estado: homologarEstadoEquidadCat(filas[i]?.estado),
        });
        ...
        if (existente) {
          ...
          if (!merge.consecutivo) {
            secuencial += 1;
            merge.consecutivo = `EQUIDAD-CAT-${año}-${mes}-${secuencial}`;
          }
          const actualizado = await EquidadCatCaso.findByIdAndUpdate(existente._id, merge, {
            new: true,
          }).lean();
          ...
          secuencial += 1;
          payload.consecutivo = `EQUIDAD-CAT-${año}-${mes}-${secuencial}`;
          if (!payload.identificacion) {
            payload.identificacion = String(payload.siniestro || payload.numeroCasoCliente || '');
          }
          const creado = await EquidadCatCaso.create(payload);
    ...
    console.error('❌ Error al importar Equidad CAT:', error);
    res.status(500).json({
      success: false,
      error: 'Error al importar el Equidad CAT',
      detalle: error.message,
    });
  }
};

const archivosListadoEquidadCat = crearControladoresArchivosListado({
  Model: EquidadCatCaso,
  nombreModulo: 'EquidadCat',
  rutaLocalPrefix: '/uploads/equidad-cat/',
});

export const subirArchivoListadoEquidadCat = archivosListadoEquidadCat.subir;
export const eliminarArchivoListadoEquidadCat = archivosListadoEquidadCat.eliminar;
